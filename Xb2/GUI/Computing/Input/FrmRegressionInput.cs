    ...
    public partial class FrmRegressionInput : FrmBase
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (confirm == DialogResult.OK)
            {
                ...
                Debug.Print("用户{0}，测项{1}是否有默认基础数据库？{2}", this.User.ID, this.RegresInput.ItemId, hasDefaultDb);
                //将原始数据加到基础数据里
                var dr = dt.NewRow();
                dr["库名"] = "原始数据";
                dr["编号"] = -1; //-1代表取原始数据
        ...
        private void RbOnCheckedChanged(object sender, EventArgs eventArgs)
        {
            ...
            if (rb.Checked)
            {
                ...
                {
                    sql = string.Format(sql, DbHelper.TnRData(), "测项编号", this.RegresInput.ItemId);
                    Debug.Print("从原始数据中查询日期：" + sql);
                }
                else
                {
                    sql = string.Format(sql, DbHelper.TnProcessedDbData(), "库编号", this.RegresInput.DatabaseId);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            this.RegresInput.InputData = dateValueList.Between(dateRange);
